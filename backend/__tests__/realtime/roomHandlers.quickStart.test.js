...
const quickStart = (socket, callback) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (targetRoom) {
            ...
            // 检查玩家是否已在房间中（重连场景）
            const existingPlayer = targetRoomState.players.find((p) => p.id === userId);
            if (existingPlayer) {
            ...
            yield socket.redis.setEx(`room:${targetRoom.id}`, 3600, JSON.stringify(targetRoomState));
            yield socket.userStateService.setUserCurrentRoom(userId, targetRoom.id);
            // 通知其他玩家
            socket.to(targetRoom.id).emit('room:player_joined', {
                player: targetRoomState.players.find((p) => p.id === userId)
            });
            return callback({
                success: true,
...
describe('roomHandlers.quickStart - 快速开始功能单元测试', () => {
    ...
    describe('加入现有房间场景', () => {
        it('应该成功加入有空位的现有房间', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成测试数据
            ...
            // 4. 验证结果  
            socketTestUtils_1.SocketTestHelper.expectSuccessCallback(mocks.callback, expect.any(Object), 'Quick start successful - joined existing room');
            socketTestUtils_1.SocketTestHelper.expectSocketJoin(mocks.socket, testData.availableRooms[0].id);
            // 5. 验证房间状态更新
            expect(mocks.redis.setEx).toHaveBeenCalledWith(`room:${testData.availableRooms[0].id}`, 3600, expect.stringContaining('"currentPlayerCount":3'));
        }));
        it('应该跳过已满员的房间', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成满员房间数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('joinExisting');
            ...
            mocks.prisma.room.findMany.mockResolvedValue([
                Object.assign(Object.assign({}, testData.availableRooms[0]), { id: 'full-room' }),
                Object.assign(Object.assign({}, testData.availableRooms[0]), { id: 'available-room' })
            ]);
            mocks.redis.get
        ...
        it('应该跳过用户已在其中的房间', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成测试数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('joinExisting');
            // 2. 同步socket数据和测试数据
            syncSocketWithTestData(mocks.socket, testData);
            ...
            mocks.prisma.room.findMany.mockResolvedValue([
                Object.assign(Object.assign({}, testData.availableRooms[0]), { id: 'user-in-room' }),
                Object.assign(Object.assign({}, testData.availableRooms[0]), { id: 'available-room' })
            ]);
            mocks.redis.get
        ...
        it('应该处理断线重连场景', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成重连场景数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('joinExisting');
            // 2. 同步socket数据和测试数据
            syncSocketWithTestData(mocks.socket, testData);
            const roomStateWithDisconnectedUser = roomStateFactory_1.RoomStateFactory.createBasicRoomState({
                players: [
                    {
                        id: testData.user.id,
            ...
            mocks.prisma.room.create.mockResolvedValue(testDataGenerator_1.TestDataGenerator.createRoomData(testData.user.id));
            // 4. 执行测试
            yield quickStart(mocks.socket, mocks.callback);
            // 4. 验证重连成功
            ...
            const reconnectedPlayer = savedRoomState.players.find((p) => p.id === testData.user.id);
            expect(reconnectedPlayer.isConnected).toBe(true);
        }));
    });
    describe('创建新房间场景', () => {
        it('应该在没有可用房间时创建新房间', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成创建新房间数据
        ...
        it('应该在所有房间都不合适时创建新房间', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成测试数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('createNew');
            const fullRoomState = roomStateFactory_1.RoomStateFactory.createFullRoomState();
            const newRoom = testDataGenerator_1.TestDataGenerator.createRoomData(testData.user.id);
            ...
            mocks.prisma.room.findMany.mockResolvedValue([
                testDataGenerator_1.TestDataGenerator.createRoomData(),
                testDataGenerator_1.TestDataGenerator.createRoomData()
            ]);
        ...
        it('应该正确初始化新房间状态', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成测试数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('createNew');
            const newRoom = testDataGenerator_1.TestDataGenerator.createRoomData(testData.user.id);
            // 2. 同步socket数据和测试数据
            ...
            // 4. 验证房间状态初始化
            const savedRoomState = JSON.parse(mocks.redis.setEx.mock.calls[0][2]);
            roomStateFactory_1.RoomStateAssertions.assertValidRoomState(savedRoomState);
            roomStateFactory_1.RoomStateAssertions.assertPlayerCount(savedRoomState, 1);
            roomStateFactory_1.RoomStateAssertions.assertRoomStatus(savedRoomState, 'WAITING');
            // 5. 验证房主设置
        ...
        it('应该正确设置新房间的默认参数', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成测试数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('createNew');
            const newRoom = testDataGenerator_1.TestDataGenerator.createRoomData(testData.user.id);
            // 2. 同步socket数据和测试数据
    ...
    describe('冲突处理场景', () => {
        it('应该处理用户已在其他房间的冲突', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成测试数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('createNew');
            // 2. 同步socket数据和测试数据
            syncSocketWithTestData(mocks.socket, testData);
        ...
        it('应该允许没有当前房间的用户快速开始', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成测试数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('createNew');
            const newRoom = testDataGenerator_1.TestDataGenerator.createRoomData(testData.user.id);
            // 2. 同步socket数据和测试数据
    ...
    describe('错误处理场景', () => {
        it('应该处理数据库查询错误', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 配置数据库错误
            mocks.userStateService.checkAndHandleRoomConflict.mockResolvedValue({
                success: true,
                code: 'NO_CURRENT_ROOM'
        ...
        it('应该处理Redis获取错误', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 生成测试数据
            const testData = testDataGenerator_1.RoomHandlerTestData.forQuickStart('joinExisting');
            // 2. 同步socket数据和测试数据
            syncSocketWithTestData(mocks.socket, testData);
        ...
        it('应该处理房间创建错误', () => __awaiter(void 0, void 0, void 0, function* () {
            // 1. 配置房间创建错误
            mocks.userStateService.checkAndHandleRoomConflict.mockResolvedValue({
                success: true,
                code: 'NO_CURRENT_ROOM'
        ...
        it('应该处理用户状态服务错误', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            mocks.prisma.room.findMany.mockResolvedValue(testData.availableRooms);
            mocks.redis.get.mockResolvedValue(JSON.stringify(testData.roomState));
            mocks.userStateService.setUserCurrentRoom.mockRejectedValue(new Error('User state service failed'));
            // 3. 执行测试
            yield quickStart(mocks.socket, mocks.callback);
            // 4. 验证错误处理
            socketTestUtils_1.SocketTestHelper.expectErrorCallback(mocks.callback, 'Internal server error');
