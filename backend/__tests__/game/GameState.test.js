...
describe('GameState', () => {
    let gameState;
    beforeEach(() => {
        gameState = new GameState_1.GameState('test-game', 10, 20, 30000);
    });
    ...
    describe('Game flow', () => {
        ...
        test('should post blinds correctly', () => {
            ...
            const smallBlindPlayer = snapshot.players.find((p) => snapshot.positions.find((pos) => pos.playerId === p.id && pos.isSmallBlind));
            const bigBlindPlayer = snapshot.players.find((p) => snapshot.positions.find((pos) => pos.playerId === p.id && pos.isBigBlind));
        ...
        test('should handle valid player actions', () => {
            ...
            expect(gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.FOLD)).toBe(true);
            const snapshot = gameState.getGameSnapshot();
            const foldedPlayer = snapshot.players.find((p) => p.id === currentPlayerId);
            expect(foldedPlayer.status).toBe(GameState_1.PlayerStatus.FOLDED);
        });
        test('should reject invalid player actions', () => {
            gameState.startNewHand();
            const currentPlayerId = gameState.getCurrentPlayerId();
            // Wrong player trying to act
            expect(gameState.executePlayerAction('wrong-player', GameState_1.PlayerAction.CALL)).toBe(false);
            // Invalid action (check when there's a bet to call)
            expect(gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.CHECK)).toBe(false);
        });
        test('should progress through game phases correctly', () => {
            ...
            let actionCount = 0;
            const maxActions = 10;
            while (currentPlayerId && gameState.getGameSnapshot().phase === GameState_1.GamePhase.PRE_FLOP && actionCount < maxActions) {
                const snapshot = gameState.getGameSnapshot();
                const currentPlayer = snapshot.players.find((p) => p.id === currentPlayerId);
                // 确定正确的行动
                let action;
                if (currentPlayer.currentBet < 20) {
                    action = GameState_1.PlayerAction.CALL;
                }
                else {
                    action = GameState_1.PlayerAction.CHECK;
                }
            ...
            expect(gameState.getGameSnapshot().phase).toBe(GameState_1.GamePhase.FLOP);
            expect(gameState.getGameSnapshot().communityCards).toHaveLength(3);
            // Complete flop betting with check actions
            actionCount = 0;
            while (currentPlayerId && gameState.getGameSnapshot().phase === GameState_1.GamePhase.FLOP && actionCount < maxActions) {
                gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.CHECK);
                currentPlayerId = gameState.getCurrentPlayerId();
                actionCount++;
            }
            expect(gameState.getGameSnapshot().phase).toBe(GameState_1.GamePhase.TURN);
            expect(gameState.getGameSnapshot().communityCards).toHaveLength(4);
            // Complete turn betting with check actions
            actionCount = 0;
            while (currentPlayerId && gameState.getGameSnapshot().phase === GameState_1.GamePhase.TURN && actionCount < maxActions) {
                gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.CHECK);
                currentPlayerId = gameState.getCurrentPlayerId();
                actionCount++;
            }
            expect(gameState.getGameSnapshot().phase).toBe(GameState_1.GamePhase.RIVER);
            expect(gameState.getGameSnapshot().communityCards).toHaveLength(5);
            // Complete river betting with check actions
            actionCount = 0;
            while (currentPlayerId && gameState.getGameSnapshot().phase === GameState_1.GamePhase.RIVER && actionCount < maxActions) {
                gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.CHECK);
                currentPlayerId = gameState.getCurrentPlayerId();
                actionCount++;
            }
            expect(gameState.getGameSnapshot().phase).toBe(GameState_1.GamePhase.FINISHED);
        });
    });
    describe('Action validation', () => {
        ...
        test('should validate call action', () => {
            const currentPlayerId = gameState.getCurrentPlayerId();
            expect(gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.CALL)).toBe(true);
        });
        test('should validate raise action', () => {
            const currentPlayerId = gameState.getCurrentPlayerId();
            expect(gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.RAISE, 40)).toBe(true);
        });
        test('should validate all-in action', () => {
            const currentPlayerId = gameState.getCurrentPlayerId();
            expect(gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.ALL_IN)).toBe(true);
            const snapshot = gameState.getGameSnapshot();
            const allInPlayer = snapshot.players.find((p) => p.id === currentPlayerId);
            expect(allInPlayer.status).toBe(GameState_1.PlayerStatus.ALL_IN);
            expect(allInPlayer.chips).toBe(0);
        });
        test('should reject invalid raise amounts', () => {
            const currentPlayerId = gameState.getCurrentPlayerId();
            // Raise too small
            expect(gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.RAISE, 1)).toBe(false);
            // Raise more than chips
            expect(gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.RAISE, 2000)).toBe(false);
        });
    });
    describe('Timeout handling', () => {
        beforeEach(() => {
            gameState = new GameState_1.GameState('test-game', 10, 20, 100); // 100ms timeout
            gameState.addPlayer('player1', 'Alice', 1000);
            gameState.addPlayer('player2', 'Bob', 1000);
            gameState.setPlayerReady('player1', true);
            gameState.setPlayerReady('player2', true);
        ...
        test('should handle timeout correctly', () => __awaiter(void 0, void 0, void 0, function* () {
            const currentPlayerId = gameState.getCurrentPlayerId();
            // Wait for timeout
            yield new Promise(resolve => setTimeout(resolve, 150));
            expect(gameState.handleTimeout(currentPlayerId)).toBe(true);
            const snapshot = gameState.getGameSnapshot();
            const timedOutPlayer = snapshot.players.find((p) => p.id === currentPlayerId);
            expect(timedOutPlayer.status).toBe(GameState_1.PlayerStatus.FOLDED);
        }));
    ...
    describe('All-in scenarios', () => {
        ...
        test('should handle all-in with insufficient chips for blinds', () => {
            ...
            const shortStackPlayer = snapshot.players.find((p) => p.chips < 100);
            if (shortStackPlayer) {
        ...
        test('should create side pots with all-in players', () => {
            ...
            while (currentPlayerId && gameState.getGameSnapshot().phase === GameState_1.GamePhase.PRE_FLOP && actionCount < maxActions) {
                ...
                // 获取当前需要跟注的金额
                const highestBet = Math.max(...snapshot.players.map((p) => p.currentBet));
                const callAmount = highestBet - player.currentBet;
                let action;
                if (player && player.chips <= 50) {
                    action = GameState_1.PlayerAction.ALL_IN;
                }
            ...
            const snapshot = gameState.getGameSnapshot();
            expect(snapshot.phase).toBe(GameState_1.GamePhase.FLOP);
    ...
    describe('Game completion and results', () => {
        ...
        test('should complete game and provide results', () => {
            ...
            let actionCount = 0;
            const maxActions = 20;
            while (currentPlayerId && gameState.getGameSnapshot().phase !== GameState_1.GamePhase.FINISHED && actionCount < maxActions) {
                const snapshot = gameState.getGameSnapshot();
                const currentPlayer = snapshot.players.find((p) => p.id === currentPlayerId);
                let action;
                if (snapshot.phase === GameState_1.GamePhase.PRE_FLOP) {
        ...
        test('should handle single player remaining (others folded)', () => {
            const currentPlayerId = gameState.getCurrentPlayerId();
            // First player folds
            gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.FOLD);
    ...
    describe('Game state management', () => {
        ...
        test('should track action history correctly', () => {
            gameState.startNewHand();
            const currentPlayerId = gameState.getCurrentPlayerId();
            gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.RAISE, 40);
            const snapshot = gameState.getGameSnapshot();
            expect(snapshot.actionHistory).toHaveLength(1);
            expect(snapshot.actionHistory[0]).toMatchObject({
                playerId: currentPlayerId,
                action: GameState_1.PlayerAction.RAISE,
                amount: 40,
                phase: GameState_1.GamePhase.PRE_FLOP
            });
        });
        test('should reset game state correctly', () => {
            ...
            gameState.reset();
            const snapshot = gameState.getGameSnapshot();
            expect(snapshot.phase).toBe(GameState_1.GamePhase.WAITING);
    ...
    describe('Edge cases', () => {
        ...
        test('should handle invalid player operations gracefully', () => {
            ...
            expect(gameState.executePlayerAction('nonexistent', GameState_1.PlayerAction.FOLD)).toBe(false);
    ...
    describe('Integration with game logic components', () => {
        ...
        test('should integrate with PotManager correctly', () => {
            const currentPlayerId = gameState.getCurrentPlayerId();
            gameState.executePlayerAction(currentPlayerId, GameState_1.PlayerAction.RAISE, 50);
            const snapshot = gameState.getGameSnapshot();
        ...
        test('should integrate with PositionManager correctly', () => {
            ...
            const dealerPosition = snapshot.positions.find((p) => p.isDealer);
            const smallBlindPosition = snapshot.positions.find((p) => p.isSmallBlind);
            const bigBlindPosition = snapshot.positions.find((p) => p.isBigBlind);
        ...
        test('should integrate with Deck correctly', () => {
            ...
            const activePlayers = snapshot.players.filter((p) => p.status === GameState_1.PlayerStatus.ACTIVE);
