...
const PositionManager_1 = require("../../src/game/PositionManager");
describe('Game Flow Integration', () => {
    ...
    beforeEach(() => {
        ...
        positionManager = new PositionManager_1.PositionManager(10, 20);
    });
    describe('Complete game simulation', () => {
        test('should simulate a full 3-player game hand', () => {
            ...
            // Evaluate hands
            const aliceResult = HandRank_1.HandEvaluator.evaluateHand([...aliceHand, ...communityCards]);
            const bobResult = HandRank_1.HandEvaluator.evaluateHand([...bobHand, ...communityCards]);
            const charlieResult = HandRank_1.HandEvaluator.evaluateHand([...charlieHand, ...communityCards]);
            // All hands should be valid
            expect(aliceResult.type).toBeGreaterThanOrEqual(HandRank_1.HandType.HIGH_CARD);
            expect(bobResult.type).toBeGreaterThanOrEqual(HandRank_1.HandType.HIGH_CARD);
            expect(charlieResult.type).toBeGreaterThanOrEqual(HandRank_1.HandType.HIGH_CARD);
            // Determine winner (simplified - just compare Alice vs Bob)
            const comparison = HandRank_1.HandEvaluator.compareHands(aliceResult, bobResult);
            let winner;
            if (comparison > 0) {
                winner = 'alice';
            }
        ...
        test('should handle specific hand comparisons correctly', () => {
            ...
            const royalFlushCards = Deck_1.Deck.parseCards(['AS', 'KS', 'QS', 'JS', 'TS', '9H', '8C']);
            const straightFlushCards = Deck_1.Deck.parseCards(['9H', '8H', '7H', '6H', '5H', 'AC', 'KD']);
            const royalFlush = HandRank_1.HandEvaluator.evaluateHand(royalFlushCards);
            const straightFlush = HandRank_1.HandEvaluator.evaluateHand(straightFlushCards);
            expect(royalFlush.type).toBe(HandRank_1.HandType.STRAIGHT_FLUSH);
            expect(straightFlush.type).toBe(HandRank_1.HandType.STRAIGHT_FLUSH);
            // Royal flush should beat straight flush
            expect(HandRank_1.HandEvaluator.compareHands(royalFlush, straightFlush)).toBeGreaterThan(0);
        });
        test('should handle wheel straight correctly', () => {
            const wheelCards = Deck_1.Deck.parseCards(['AS', '2H', '3D', '4C', '5S', 'KH', 'QC']);
            const wheelResult = HandRank_1.HandEvaluator.evaluateHand(wheelCards);
            expect(wheelResult.type).toBe(HandRank_1.HandType.STRAIGHT);
    ...
    describe('Edge cases and error handling', () => {
        ...
        test('should handle invalid hand evaluation input', () => {
            expect(() => {
                HandRank_1.HandEvaluator.evaluateHand(Deck_1.Deck.parseCards(['AS', 'KH', 'QD'])); // Only 3 cards
            }).toThrow();
            expect(() => {
                HandRank_1.HandEvaluator.evaluateHand(Deck_1.Deck.parseCards(['AS', 'KH', 'QD', 'JS', '10H', '9C', '8S', '7D'])); // 8 cards
            }).toThrow();
        });
