...
class PositionManager {
    constructor(smallBlind, bigBlind) {
        this.players = []; // 玩家ID列表，按座位顺序
        this.dealerIndex = 0; // 庄家位置索引
        this.smallBlindAmount = 0; // 小盲金额
        this.bigBlindAmount = 0; // 大盲金额
     ...
     */
    setPlayers(players, initialDealerIndex = 0) {
        if (players.length < 2) {
     ...
     * 获取翻牌前的下注顺序（大盲后开始）
     */
    getPreflopBettingOrder(excludeAllIn = []) {
        if (this.players.length < 2) {
            return [];
        }
        const availablePlayers = this.players.filter(id => !excludeAllIn.includes(id));
     ...
     * 更新盲注金额
     */
    updateBlindAmounts(smallBlind, bigBlind) {
