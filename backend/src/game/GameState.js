...
exports.GameState = exports.PlayerStatus = exports.PlayerAction = exports.GamePhase = void 0;
...
(function (GamePhase) {
    ...
    GamePhase["FINISHED"] = "finished"; // 游戏结束
...
(function (PlayerStatus) {
    ...
    PlayerStatus["SITTING_OUT"] = "sitting_out"; // 暂离
})(PlayerStatus || (exports.PlayerStatus = PlayerStatus = {}));
/**
 * 游戏状态管理器
 */
class GameState {
    constructor(gameId, smallBlind = 10, bigBlind = 20, actionTimeout = 30000) {
        ...
        this.actionTimeoutMs = 30000; // 30秒操作超时
        this.isHandInProgress = false;
        /**
         * 检查游戏阶段是否发生变化（临时标记）
         */
        this.lastKnownPhase = GamePhase.WAITING;
        this.gameId = gameId;
        this.deck = new Deck_1.Deck();
        this.potManager = new PotManager_1.PotManager();
        this.positionManager = new PositionManager_1.PositionManager(smallBlind, bigBlind);
        this.actionTimeoutMs = actionTimeout;
    }
    /**
     * 添加玩家到游戏
     */
    addPlayer(playerId, playerName, chips) {
     ...
     * 验证操作是否有效
     */
    isValidAction(playerId, action, amount) {
        const player = this.players.get(playerId);
        if (!player)
            return false;
    ...
    showdown() {
        ...
        for (const player of activePlayers) {
            ...
            handRanks.set(player.id, HandRank_1.HandEvaluator.evaluateHand(allCards));
        }
        // 为每个筹码池确定赢家
        const pots = this.potManager.getPots();
        const winners = {};
        for (const pot of pots) {
            const eligiblePlayers = pot.eligiblePlayers.filter(id => activePlayers.some(p => p.id === id));
            ...
            for (const playerId of eligiblePlayers) {
                ...
                if (!bestHand || HandRank_1.HandEvaluator.compareHands(hand, bestHand) > 0) {
                    bestHand = hand;
                    potWinners = [playerId];
                }
                else if (HandRank_1.HandEvaluator.compareHands(hand, bestHand) === 0) {
     ...
     * 执行玩家操作（别名方法）
     */
    makeAction(playerId, action, amount = 0) {
