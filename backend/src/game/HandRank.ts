...
export interface HandRank {
  type: HandType;           // 牌型类型
  primaryRank: number;      // 主要比较点数
  secondaryRank?: number;   // 次要比较点数
  kickers: number[];        // 踢脚牌（用于平局时比较）
  cards: Card[];           // 组成这个牌型的5张牌
  name: string;            // 牌型名称
}
...
export class HandEvaluator {
   ...
   * 评估5张牌的牌型
   */
  private static evaluate5Cards(cards: Card[]): HandRank {
    if (cards.length !== 5) {
    ...
    if (isFourOfAKind) return isFourOfAKind;

    const isFullHouse = this.isFullHouse(sortedCards);
    if (isFullHouse) return isFullHouse;
    ...
    if (isThreeOfAKind) return isThreeOfAKind;

    const isTwoPair = this.isTwoPair(sortedCards);
    if (isTwoPair) return isTwoPair;

    const isOnePair = this.isOnePair(sortedCards);
    if (isOnePair) return isOnePair;
   ...
   * 检查是否为顺子
   */
  private static isStraight(cards: Card[]): HandRank | null {
    const straightInfo = this.checkStraight(cards);
    if (straightInfo) {
      return {
   ...
   * 检查是否为一对
   */
  private static isOnePair(cards: Card[]): HandRank | null {
    const rankCounts = this.getRankCounts(cards);
    const pairRank = Object.keys(rankCounts).find(rank => rankCounts[parseInt(rank)] === 2);
   ...
   * 高牌
   */
  private static isHighCard(cards: Card[]): HandRank {
    const ranks = cards.map(card => card.rank).sort((a, b) => b - a);
    return {
   ...
   * 检查是否为顺子
   */
  private static checkStraight(cards: Card[]): { highCard: number } | null {
