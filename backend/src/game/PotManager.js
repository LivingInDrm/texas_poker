...
class PotManager {
    constructor() {
        this.pots = [];
        this.playerBets = new Map(); // 每个玩家在当前轮的总下注
        this.reset();
    }
    /**
     * 重置所有筹码池
    ...
    calculatePots(activePlayers) {
        ...
        for (let i = 0; i < bets.length; i++) {
            ...
            // 只包含还在游戏中的玩家
            const eligiblePlayers = playersInThisLayer.filter(playerId => activePlayers.includes(playerId));
     ...
     */
    copyFrom(other) {
        this.pots = other.pots.map(pot => (Object.assign(Object.assign({}, pot), { eligiblePlayers: [...pot.eligiblePlayers] })));
